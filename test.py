...
def test(img_id,model)#(obj_name, model, opt, base_dir):
	...
	for st in range(MAX_SENTC): # Iterate over each sentence separately
		...
		if st == 0: # Initialize the hidden state for the first se
			temp_hid = np.zeros(hidden_size, dtype = np.float32) # random.uniform(0, 1, (opt.hidden_size - star_embed ) )
		...
		if strtstp_ni == 0: # So we continue
			val_sent = val_sent + 1
			gl_mh[0, 0, :, st] = (model(model_hidden_st, None, 'topic')[0].cpu().data.numpy()).reshape(1, 1, hidden_size) # Transform the hidden state to obtain the topic vector
